...
TEST_F(PresetTest, TunerDisplay_MuteToggleHover_WithTooltip_ShowsTooltip) {
    ...
    // We cannot easily assert tooltip contents in ImGui 1.89+ headless cleanly without internal
    // API, but running the branch provides the coverage!
    ImGui::End();
}
...
TEST_F(PresetTest, CabinetDisplay_IRName_LongName_IsTruncatedTo20Chars) {
    ...
    if (f) {
        ...
        const uint8_t wav_header[44] = {'R',  'I',  'F',  'F',  0x24, 0x00, 0x00, 0x00, 'W',
                                        'A',  'V',  'E',  'f',  'm',  't',  ' ',  0x10, 0x00,
...
TEST_F(PresetTest, LooperDisplay_LevelSlider_Change_ClampsAndPushesToEngine) {
    ...
    ImGuiIO& io = ImGui::GetIO();
    float slider_y = p0.y + 55 * 1.0f + 18 * 1.0f + 16 * 1.0f + 22.0f * 1.0f + 6 * 1.0f +
                     22.0f * 1.0f + 8 * 1.0f + 5.0f;

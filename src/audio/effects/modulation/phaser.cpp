...
Phaser::Phaser() {
    params_ = {
        {"Rate", 0.5f, 0.05f, 10.0f, 0.5f, "Hz",
         "LFO speed. Low values create a slow, hypnotic sweep; high values create a fast "
         "vibrato-like effect."},
        ...
        {"Stages", 0.0f, 0.0f, 3.0f, 0.0f, "",
         "Number of all-pass stages: 0=4 (Phase 90), 1=6, 2=8, 3=12. More stages add more "
         "notches."},
        {"Feedback", 0.5f, 0.0f, 0.95f, 0.5f, "",
         "Feeds the chain output back to the input, adding resonance and intensity to the phasing "
         "notches."},
...
void Phaser::process_stereo(float* left, float* right, int num_samples) {
    ...
    for (int i = 0; i < num_samples; ++i) {
        ...
        const float fc_l = clamp(200.0f * std::exp(lfo_l * depth * log_ratio), 80.0f,
                                 static_cast<float>(sample_rate_) * 0.40f);
        ...
        const float fc_r = clamp(200.0f * std::exp(lfo_r * depth * log_ratio), 80.0f,
                                 static_cast<float>(sample_rate_) * 0.40f);

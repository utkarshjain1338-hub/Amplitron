...
Flanger::Flanger() {
    params_ = {
        ...
        {"Depth", 2.0f, 0.1f, 7.0f, 2.0f, "ms",
         "Modulation depth in milliseconds. Controls the width of the delay time sweep around the "
         "base delay."},
        {"Delay", 1.0f, 0.1f, 10.0f, 1.0f, "ms",
         "Base delay time. Short values (< 2ms) create tight comb filtering; longer values soften "
         "the effect."},
        {"Feedback", 0.7f, -0.95f, 0.95f, 0.7f, "",
         "Feedback amount. Positive values add metallic resonance; negative values create a "
         "hollow, phasey character."},
        {"Mix", 0.5f, 0.0f, 1.0f, 0.5f, "",
         "Dry/wet blend. At 0.5 the comb filter notches are deepest for the classic flanger "
         "sound."},
    };
...
void Flanger::process(float* buffer, int num_samples) {
    ...
    for (int i = 0; i < num_samples; ++i) {
        ...
        const float delay_samples =
            clamp((delay_ms + lfo * depth_ms) * 0.001f * static_cast<float>(sample_rate_), 1.0f,
...
void Flanger::process_stereo(float* left, float* right, int num_samples) {
    ...
    for (int i = 0; i < num_samples; ++i) {
        ...
        const float delay_samp_l =
            clamp((delay_ms + lfo_l * depth_ms) * 0.001f * static_cast<float>(sample_rate_), 1.0f,
                  static_cast<float>(max_delay_samples_ - 2));
        ...
        const float delay_samp_r =
            clamp((delay_ms + lfo_r * depth_ms) * 0.001f * static_cast<float>(sample_rate_), 1.0f,
                  static_cast<float>(max_delay_samples_ - 2));

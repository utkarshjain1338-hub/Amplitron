...
Distortion::Distortion() {
    params_ = {
        {"Drive", 2.0f, 1.0f, 20.0f, 2.0f, "x",
         "Amount of input gain slamming into the hard clippers. Creates aggressive, dense harmonic "
         "distortion."},
        {"Tone", 0.6f, 0.0f, 1.0f, 0.6f, "",
         "Low-pass filter cutoff to tame the fizzy high-end generated by hard clipping."},
        {"Level", 0.5f, 0.0f, 1.0f, 0.5f, "",
         "Master output volume of the pedal. Useful to match volume with the pedal bypassed."},
    };
...
void Distortion::process(float* buffer, int num_samples) {
    ...
    for (int i = 0; i < num_samples; ++i) {
        ...
        float hard_amount =
            (drive - params_[0].min_val) / (params_[0].max_val - params_[0].min_val);

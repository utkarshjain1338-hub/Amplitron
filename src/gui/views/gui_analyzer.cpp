...
// ─────────────────────────────────────────────────────────────────────────────
void GuiAnalyzer::render_vu_bar(const char* id, const char* label, float rms_level, float peak_hold,
                                bool clip_active, float clip_flash, ImU32 base_color,
                                ImU32 peak_color) {
    ImGui::PushID(id);
    ...
    dl->AddLine(ImVec2(peak_x, pos.y - 1.0f), ImVec2(peak_x, pos.y + height + 1.0f), peak_color,
                2.0f);

    if (clip_active || clip_flash > 0.01f)
        dl->AddText(ImVec2(pos.x + width - 32.0f, pos.y - 1.0f), IM_COL32(255, 90, 90, 255),
                    "CLIP");
...
// ─────────────────────────────────────────────────────────────────────────────
void GuiAnalyzer::draw_spectrum(ImDrawList* dl, const ImVec2& pos, const ImVec2& size) const {
    ...
    const auto draw_set = [&](const std::array<float, BARS>& bars,
        ...
        for (int i = 0; i < BARS; ++i) {
            ...
            dl->AddRectFilled(ImVec2(center - half, y), ImVec2(center + half, pmax.y), bar_col,
                              1.5f);

            const float peak_t = (clamp(peaks[i], kDisplayMinDb, kDisplayMaxDb) - kDisplayMinDb) /
...
void GuiAnalyzer::render() {
    ...
    render_vu_bar("input_vu", "INPUT RMS", p.smoothed_input_rms, p.input_peak_hold,
                  p.input_clip_active, p.input_clip_flash, IM_COL32(60, 200, 110, 230),
                  IM_COL32(255, 230, 120, 255));
    ImGui::NextColumn();
    render_vu_bar("output_vu", "OUTPUT RMS", p.smoothed_output_rms, p.output_peak_hold,
                  p.output_clip_active, p.output_clip_flash, IM_COL32(80, 170, 245, 230),
                  IM_COL32(255, 230, 120, 255));
    ImGui::Columns(1);

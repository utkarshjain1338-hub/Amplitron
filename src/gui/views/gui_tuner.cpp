...
void GuiTuner::render(bool& show) {
    ...
    if (p.has_signal && p.note_idx >= 0 && p.note_name_fn) {
        ...
        dl->AddText(ImGui::GetFont(), ImGui::GetFontSize() * scale, note_pos, Theme::TEXT_PRIMARY,
                    note_buf);
        ...
        bar_pos.x += 20;
        dl->AddRectFilled(bar_pos, ImVec2(bar_pos.x + bar_w, bar_pos.y + bar_h), Theme::KNOB_BG,
                          4.0f);

        float cx = bar_pos.x + bar_w * 0.5f;
        dl->AddLine(ImVec2(cx, bar_pos.y - 2), ImVec2(cx, bar_pos.y + bar_h + 2), Theme::TEXT_DIM,
                    2.0f);
        ...
        dl->AddRectFilled(ImVec2(needle_x - 4, bar_pos.y - 3),
                          ImVec2(needle_x + 4, bar_pos.y + bar_h + 3), needle_col, 3.0f);

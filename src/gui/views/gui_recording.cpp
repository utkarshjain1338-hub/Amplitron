...
void GuiRecording::render() {
    ...
    if (p.is_recording) {
        ...
        ImGui::TextColored(peak > 0.9f   ? ImVec4(1, 0.2f, 0.2f, 1)
                           ...
                           "  Peak: %.1f dB", peak > 0.0001f ? 20.0f * std::log10(peak) : -96.0f);
        ...
        draw->AddRectFilled(wave_pos, ImVec2(wave_pos.x + wave_w, wave_pos.y + wave_h),
                            IM_COL32(20, 18, 16, 255), 4.0f);

        float center_y = wave_pos.y + wave_h * 0.5f;
        draw->AddLine(ImVec2(wave_pos.x, center_y), ImVec2(wave_pos.x + wave_w, center_y),
        ...
        ImGui::SameLine();
        ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), "  Ready to record  |  WAV 16-bit %d Hz",
                           p.sample_rate);
    }

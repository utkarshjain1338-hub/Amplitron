...
struct AddGraphNodeCommand : public Command {
    ...
    AddGraphNodeCommand(IAudioEngine& engine, const std::string& name, EffectType type,
                        std::shared_ptr<Effect> pedal, ImVec2 pos, int num_inputs = 0)
        : engine_(engine),
          name(name),
          type(type),
          pedal(pedal),
...
struct RemoveGraphNodeCommand : public Command {
    ...
    bool execute() override {
        ...
        for (const auto& link : engine_.graph().get_links()) {
            if (std::find(cached_node.input_pin_ids.begin(), cached_node.input_pin_ids.end(),
                          link.dest_pin_id) != cached_node.input_pin_ids.end() ||
                std::find(cached_node.output_pin_ids.begin(), cached_node.output_pin_ids.end(),
...
struct AddGraphLinkCommand : public Command {
    ...
    AddGraphLinkCommand(IAudioEngine& engine, int src_pin, int dst_pin) : engine_(engine) {
    ...
    bool execute() override {
        if (link.id == -1) {
            ...
            if (auto_added_pin_node_id != -1) {
                engine_.graph().restore_input_pin(auto_added_pin_node_id, auto_added_pin_id,
                                                  auto_added_pin_index, auto_added_pin_gain);
            }
            if (auto_added_out_pin_node_id != -1) {
                engine_.graph().restore_output_pin(auto_added_out_pin_node_id,
                                                   auto_added_out_pin_id, auto_added_out_pin_index);
            }
            engine_.graph().restore_link(link);
        }
        if (was_successful) {
...
struct RemoveGraphLinkCommand : public Command {
    ...
    RemoveGraphLinkCommand(IAudioEngine& engine, const GraphLink& l) : engine_(engine), link(l) {}
                        ...
                        for (size_t i = 0; i < node->input_pin_ids.size(); ++i) {
                            if (node->input_pin_ids[i] == link.dest_pin_id) {
                                ...
                                auto_removed_pin_gain =
                                    (i < node->input_gains.size()) ? node->input_gains[i] : 1.0f;
                                break;
                            }
                        }
                        if (engine_.graph().remove_input_pin(dest_node_id, link.dest_pin_id)) {
    ...
    void undo() override {
        if (auto_removed_pin_node_id != -1) {
            engine_.graph().restore_input_pin(auto_removed_pin_node_id, auto_removed_pin_id,
                                              auto_removed_pin_index, auto_removed_pin_gain);
        }
        if (auto_removed_out_pin_node_id != -1) {
            engine_.graph().restore_output_pin(auto_removed_out_pin_node_id,
                                               auto_removed_out_pin_id, auto_removed_out_pin_index);
        }
        engine_.graph().restore_link(link);
        engine_.commit_graph_changes();
    }

...
class LoadPresetCommand : public Command {
   public:
     ...
     */
    LoadPresetCommand(IAudioEngine &engine, std::vector<EffectSnapshot> before_state,
                      float before_input_gain, float before_output_gain,
                      std::vector<EffectSnapshot> after_state, float after_input_gain,
                      float after_output_gain)
        : engine_(engine),
    ...
    void undo() override { apply_state(before_state_, before_input_gain_, before_output_gain_); }
...
class RecallSnapshotCommand : public Command {
   public:
    ...
    RecallSnapshotCommand(IAudioEngine &engine, std::vector<EffectSnapshot> before_effects,
                          float before_input_gain, float before_output_gain,
                          std::vector<EffectSnapshot> after_effects, float after_input_gain,
                          float after_output_gain)
        : engine_(engine),
    ...
    void undo() override { apply_state(before_effects_, before_input_gain_, before_output_gain_); }
